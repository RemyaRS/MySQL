# Windowing - groupig and performing an operation
...
#Aggregated windowing - sum, min, max, avg, count function comes under this category
#here group by is helping to create a window
...
#To get top result - 1 or 0,1  should be written after desc limit
...
select * from ineuron_students where student_batch = 'fsda' order by students_marks desc limit 2;

select * from ineuron_students where student_batch = 'fsda' order by students_marks desc limit 1,1;

#To get 2 records staring from top 3
...
# To get 3 records staring from index 2 or 3rd position 
...
row_number() over(order by students_marks) as 'row_number' from ineuron_students;
 ...
 # Analytical windowing - over and partition by keywords are used for it
...
# Difference between row rank and dense rank :
# Row rank follows -row_number and skips some ranks 
# Dense rank - never skips a rank
